(ns staffing.operation
  "OperationActor — one placement/extension/timesheet/report/dispute
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (TempStaffing-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the StaffingGovernor
  (:govern) and the rollout phase gate (:decide) before anything commits to
  the SSoT or is disclosed.
  ...
  decision to a human reviewer (staffing coordinator / payroll officer).
...
(defn build
  ...
  `staffing.store/Store`).
  opts:
    :advisor      — a `staffing.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TempStaffing-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; StaffingGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
      ...
      ;; (staffing coordinator / payroll officer) resumes with :approval.
